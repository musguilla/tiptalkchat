  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Videollamadas y llamadas de voz activas ahora mismo. Entras como observador invisible: los participantes no ven que estás mirando."
      ...
      {data && !data.configured ? (
        <EmptyState
          ...
          title="LiveKit no está configurado como SFU"
          hint="Las cámaras en directo requieren SFU_PROVIDER=livekit con sus credenciales. Con el modo P2P el vídeo va navegador a navegador y no se puede observar desde el servidor."
        />
      ) : loading ? (
        ...
        <EmptyState
          ...
          title="No hay ninguna llamada en directo"
          hint="Cuando alguien inicie una videollamada o llamada de voz, aparecerá aquí al instante."
        />
      ) : (
      ...
      {data && data.configured && !data.recordingAvailable && calls.length > 0 && (
        <p className="mt-4 rounded-md border border-amber-200 bg-amber-50 px-4 py-3 text-xs text-amber-800">
          La grabación de sesiones está desactivada. Para poder grabar, añade las claves
      ...
      <div className="flex flex-1 flex-col gap-3 p-4">
        <div className="min-w-0">
          <p className="flex items-center gap-2 truncate font-display font-bold text-ink">
            {call.name}
            {call.closed && <Badge tone="neutral">cerrada</Badge>}
          </p>
